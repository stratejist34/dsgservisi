---
title: "Passat 120 Bin Bakımı"
description: "Volkswagen Passat 120 bin kilometre bakımı: Timing zincir, DSG kavrama, su pompası, alternatör kontrolü. 120.000 km ağır bakım paketi hizmetleri ve içeriği."
category: "Volkswagen"
tags: ["volkswagen", "passat", "120 bin bakım", "120.000 km bakım", "ağır bakım", "timing zincir", "dsg kavrama"]
slug: "passat-120-bin-bakimi"
publishDate: "2025-11-17T12:50:00+03:00"
updatedDate: "2025-11-17T12:50:00+03:00"
featuredImage: "/images/blog/passat-120-bin-bakimi.jpg"
imageAlt: "Volkswagen Passat 120 bin kilometre bakımı - Yıldızlar Grup DSG Servisi"
ogImage: "/images/blog/passat-120-bin-bakimi.jpg"
author: "DSG Servisi"
draft: false

# Hizmet Şeması
serviceName: "Volkswagen Passat 120 Bin Bakımı"
serviceType: "Araç Bakımı"
serviceDescription: "Volkswagen Passat araçlarında 120.000 km ağır bakım hizmeti. Timing zincir, DSG kavrama, su pompası kontrolü."
areaServed: ["İstanbul", "Beylikdüzü", "Büyükçekmece", "Esenyurt", "Avcılar"]

---

Yıldızlar Grup, Volkswagen Passat araçlarında 120.000 km ağır bakım hizmeti sunan uzman servis merkezidir. 120 bin kilometre bakımı, Passat'ın en kapsamlı bakımıdır ve timing zincir kontrolü, DSG kavrama kontrolü, su pompası kontrolü gibi kritik işlemleri içerir. 15+ yıllık deneyimimizle yetkili servise alternatif, %50-70 daha ekonomik 120 bin bakım fiyatları sunuyoruz.

Volkswagen Passat 120 bin bakımı, aracın ömrünü uzatan ve büyük arızaları önleyen en kritik bakım işlemlerinden biridir. Timing zincir kontrolü, DSG kavrama kontrolü, su pompası kontrolü, alternatör kontrolü ve radyatör kontrolü gibi işlemler 120.000 km'de mutlaka yapılmalıdır. Yetkili servis fiyatlarına göre %50-70 daha ekonomik 120 bin bakım hizmetiyle Passat aracınızın performansını koruyun.

:::tip

**UZMAN TAVSİYESİ:** Passat 120 bin bakımında erken müdahale %50'ye varan maliyet tasarrufu sağlar. Timing zincir kontrolü 100.000 km'de mutlaka yapılmalıdır. Değişim gerekiyorsa uygun maliyetler; koparsa motor hasarı uygun maliyetler. DSG kavrama kontrolü 120.000 km'de yapılmalı, değişim gerekiyorsa uygun maliyetler. Su pompası kontrolü 120.000 km'de yapılmalı, değişim gerekiyorsa uygun maliyetler. Alternatör kontrolü 120.000 km'de yapılmalı, değişim gerekiyorsa uygun maliyetler.

:::

:::cta

**Volkswagen Passat 120 bin bakım için hızlı teklif alın:**

- [📞 0533 262 34 51](tel:05332623451)
- [WhatsApp Randevu](https://wa.me/905332623451)
- [Konum Tarifi](https://maps.app.goo.gl/vmZyp6qu3pCgE8vRA)

:::

## Passat 120 Bin Bakımı Hizmetleri

| Bakım Paketi | İçerik | Toplam Fiyat | Süre |
| --- | --- | ---: | --- |
| 120.000 km Ağır Bakım Paketi | 90.000 km + timing zincir, DSG kavrama, su pompası | 📞 Fiyat Sorunuz | 2-3 gün |
| 120.000 km Temel Bakım | Filtreler, yağ, fren, klima | 📞 Fiyat Sorunuz | 1 gün |
| 120.000 km Kapsamlı Bakım | Temel + DSG yağ, DPF, AdBlue | 📞 Fiyat Sorunuz | 1-2 gün |

## 120.000 km Ağır Bakım Paketi İçeriği

### Önceki Bakımların Özeti

120.000 km bakımı, 90.000 km bakımını da içerir. Önceki bakımlarda yapılan işlemler:

| Kilometre | Yapılan İşlemler |
| --- | --- |
| 60.000 km | Motor yağı, filtreler, fren kontrolü, klima bakımı |
| 90.000 km | 60.000 km + DSG yağ değişimi, DPF temizleme, turbo kontrolü, AdBlue |

### 120.000 km'de Yapılan Yeni İşlemler

120.000 km bakımında yapılan kritik işlemler:

| İşlem | Maliyet | Açıklama | Zorunluluk |
| --- | ---: | --- | --- |
| Timing zincir kontrolü | Servisimizde | 2.0 TDI timing zincir gerginlik kontrolü | Zorunlu |
| Timing zincir değişimi (gerekirse) | 📞 Fiyat Sorunuz | Timing zincir ve gergi değişimi | Kontrole göre |
| DSG kavrama kontrolü | Servisimizde | DQ250/DQ381 kavrama aşınma kontrolü | Zorunlu |
| DSG kavrama değişimi (gerekirse) | 📞 Fiyat Sorunuz | DQ250/DQ381 kavrama değişimi | Kontrole göre |
| Su pompası kontrolü | Servisimizde | Su pompası ve termostat kontrolü | Zorunlu |
| Su pompası değişimi (gerekirse) | 📞 Fiyat Sorunuz | Su pompası ve termostat değişimi | Kontrole göre |
| Alternatör kontrolü | Servisimizde | Alternatör şarj kontrolü | Zorunlu |
| Alternatör değişimi (gerekirse) | 📞 Fiyat Sorunuz | Alternatör değişimi | Kontrole göre |
| Radyatör kontrolü | Servisimizde | Radyatör sızıntı kontrolü | Zorunlu |
| Radyatör değişimi (gerekirse) | 📞 Fiyat Sorunuz | Radyatör değişimi | Kontrole göre |

## Kritik Bakım İşlemleri Detayları

### Timing Zincir Kontrolü ve Değişimi

2.0 TDI motorlarda timing zincir kontrolü 120.000 km'de kritiktir.

| İşlem | Maliyet | Süre | Açıklama |
| --- | ---: | --- | --- |
| Timing zincir kontrolü | Servisimizde | 30 dakika | Gerginlik ve aşınma kontrolü |
| Timing zincir değişimi | 📞 Fiyat Sorunuz | 2-3 gün | Zincir, gergi, kılavuzlar |
| Motor hasarı (zincir koparsa) | 📞 Fiyat Sorunuz | 5-7 gün | Piston, valf, silindir kapağı |

**Timing Zincir Önemi:**
- Zincir koparsa motor hasarı uygun maliyetler
- Erken değişim maliyeti uygun maliyetler
- 2010-2015 arası modellerde gerginlik kontrolü önemli
- kontrol servisimizde yapılır, değişim gerekiyorsa maliyet uygun maliyetler

**Timing Zincir Değişim Belirtileri:**
- Motor çalışırken metalik ses
- Motor performansında düşüş
- Yakıt tüketiminde artış
- Motor arıza lambası

### DSG Kavrama Kontrolü ve Değişimi

DQ250/DQ381 ıslak kavrama kontrolü 120.000 km'de kritiktir.

| İşlem | Maliyet | Süre | Açıklama |
| --- | ---: | --- | --- |
| DSG kavrama kontrolü | Servisimizde | 30 dakika | Kavrama aşınma kontrolü |
| DSG kavrama değişimi | 📞 Fiyat Sorunuz | 2-3 gün | DQ250/DQ381 kavrama değişimi |
| Mekatronik hasarı (geç kalınırsa) | 📞 Fiyat Sorunuz | 3-5 gün | Kavrama + mekatronik |

**DSG Kavrama Önemi:**
- Kavrama aşınması şanzıman arızasına neden olur
- Erken değişim maliyeti uygun maliyetler
- Geç kalınırsa mekatronik hasarı eklenince uygun maliyetler
- Şehir içi sık dur-kalkta kavrama aşınması %30 hızlanır

**DSG Kavrama Değişim Belirtileri:**
- Vites geçişlerinde titreme
- Kalkışta sarsıntı
- Vites geçişlerinde gecikme
- Şanzıman arıza lambası

### Su Pompası Kontrolü ve Değişimi

Su pompası kontrolü 120.000 km'de kritiktir.

| İşlem | Maliyet | Süre | Açıklama |
| --- | ---: | --- | --- |
| Su pompası kontrolü | Servisimizde | 15 dakika | Pompa ve termostat kontrolü |
| Su pompası değişimi | 📞 Fiyat Sorunuz | Aynı gün | Pompa ve termostat değişimi |
| Motor hasarı (pompa arızası) | 📞 Fiyat Sorunuz | 2-3 gün | Soğutma sistemi hasarı |

**Su Pompası Önemi:**
- Pompa arızası motor aşırı ısınmasına neden olur
- Erken değişim maliyeti uygun maliyetler
- Motor hasarı durumunda uygun maliyetler
- Termostat değişimi dahil

**Su Pompası Değişim Belirtileri:**
- Motor aşırı ısınması
- Soğutma suyu kaçağı
- Termostat arızası
- Motor sıcaklık göstergesi yükselmesi

### Alternatör Kontrolü ve Değişimi

Alternatör kontrolü 120.000 km'de kritiktir.

| İşlem | Maliyet | Süre | Açıklama |
| --- | ---: | --- | --- |
| Alternatör kontrolü | Servisimizde | 15 dakika | Şarj ve voltaj kontrolü |
| Alternatör değişimi | 📞 Fiyat Sorunuz | Aynı gün | Alternatör değişimi |
| Akü hasarı (alternatör arızası) | 📞 Fiyat Sorunuz | Aynı gün | Akü değişimi gerekebilir |

**Alternatör Önemi:**
- Alternatör arızası akü şarjını engeller
- Erken değişim maliyeti uygun maliyetler
- Akü hasarı durumunda ek maliyet uygun maliyetler
- kontrol servisimizde yapılır

**Alternatör Değişim Belirtileri:**
- Akü şarj uyarısı
- Farlarda zayıflama
- Elektrikli sistemlerde sorun
- Motor çalışırken akü voltajı düşüşü

### Radyatör Kontrolü ve Değişimi

Radyatör kontrolü 120.000 km'de kritiktir.

| İşlem | Maliyet | Süre | Açıklama |
| --- | ---: | --- | --- |
| Radyatör kontrolü | Servisimizde | 15 dakika | Sızıntı ve tıkanma kontrolü |
| Radyatör değişimi | 📞 Fiyat Sorunuz | Aynı gün | Radyatör değişimi |
| Motor hasarı (radyatör arızası) | 📞 Fiyat Sorunuz | 2-3 gün | Soğutma sistemi hasarı |

**Radyatör Önemi:**
- Radyatör arızası motor aşırı ısınmasına neden olur
- Erken değişim maliyeti uygun maliyetler
- Motor hasarı durumunda uygun maliyetler
- kontrol servisimizde yapılır

## Motor Tipine Göre 120 Bin Bakım Hizmetleri

### 2.0 TSI Motor (Benzin) 120 Bin Bakım

| İşlem | Maliyet | Süre |
| --- | ---: | --- |
| 90.000 km bakım paketi | 📞 Fiyat Sorunuz | 1-2 gün |
| DSG kavrama kontrolü | Servisimizde | 30 dakika |
| DSG kavrama değişimi (gerekirse) | 📞 Fiyat Sorunuz | 2-3 gün |
| Su pompası kontrolü | Servisimizde | 15 dakika |
| Su pompası değişimi (gerekirse) | 📞 Fiyat Sorunuz | Aynı gün |
| Alternatör kontrolü | Servisimizde | 15 dakika |
| Alternatör değişimi (gerekirse) | 📞 Fiyat Sorunuz | Aynı gün |
| **TOPLAM** | **güncel fiyatlar** | **2-3 gün** |

**2.0 TSI Özel Notlar:**
- DSG kavrama kontrolü 120.000 km'de mutlaka yapılmalı
- Su pompası kontrolü 120.000 km'de yapılmalı
- Alternatör kontrolü 120.000 km'de yapılmalı

### 2.0 TDI Motor (Dizel) 120 Bin Bakım

| İşlem | Maliyet | Süre |
| --- | ---: | --- |
| 90.000 km bakım paketi | 📞 Fiyat Sorunuz | 1-2 gün |
| Timing zincir kontrolü | Servisimizde | 30 dakika |
| Timing zincir değişimi (gerekirse) | 📞 Fiyat Sorunuz | 2-3 gün |
| DSG kavrama kontrolü | Servisimizde | 30 dakika |
| DSG kavrama değişimi (gerekirse) | 📞 Fiyat Sorunuz | 2-3 gün |
| Su pompası kontrolü | Servisimizde | 15 dakika |
| Su pompası değişimi (gerekirse) | 📞 Fiyat Sorunuz | Aynı gün |
| Alternatör kontrolü | Servisimizde | 15 dakika |
| Alternatör değişimi (gerekirse) | 📞 Fiyat Sorunuz | Aynı gün |
| **TOPLAM** | **güncel fiyatlar** | **2-3 gün** |

**2.0 TDI Özel Notlar:**
- Timing zincir kontrolü 120.000 km'de mutlaka yapılmalı
- Timing zincir koparsa motor hasarı uygun maliyetler
- DSG kavrama kontrolü 120.000 km'de yapılmalı
- AdBlue sistemi kontrolü 90.000 km'de yapılmış olmalı

## 120 Bin Bakım Süreci

1. **Ön Kontrol:** Araç detaylı kontrol edilir (araç başında)
2. **Fiyat Teklifi:** Bakım maliyeti yazılı olarak sunulur
3. **Onay:** Müşteri onayı sonrası bakım başlar
4. **Kontroller:** Timing zincir, DSG kavrama, su pompası, alternatör kontrolü
5. **Değişimler:** Gerekli parçalar değiştirilir
6. **Test:** Araç test edilerek kontrol edilir
7. **Teslimat:** Araç müşteriye teslim edilir

## Neden Yıldızlar Grup?

- **15+ Yıl Deneyim:** Passat 120 bin bakımında uzman ekip
- **Orijinal Parça:** Tüm parçalar orijinal ve sertifikalı
- **Şeffaf Fiyatlandırma:** Ön kontrol servisimizde yapılır, servis teklifi yazılı
- **Hızlı Çözüm:** 2-3 gün içinde teslimat
- **Garanti:** Tüm işçiliklerde 12 ay, parçalarda 2 yıl garanti
- **6.500+ Mutlu Müşteri:** İstanbul'un en güvenilir Volkswagen servisi

## Müşteri Yorumları

<div class="review-card">
  <div class="review-avatar">FK</div>
  <div class="review-content">
    <div class="review-meta">
      <strong>Furkan Kaplan</strong>
      <span class="review-stars">★★★★★</span>
      <span class="review-date">bir yıl önce</span>
    </div>
    <p class="review-text">Ömer usta ve Genç ekibine çok Tesekkür ediyorum. 1 saat icinde sorunu cözdüler. Yetkili servise gidecegime buraya giderim daha iyi. Muhasebedeki abladan tutun çiraklara kadar herkes çok güzel ilgilendi. Özellikle Gurbetci Kardeslerimize sesleniyorum yola cikmadan önce buraya ugrayip bakiminizi yapabilirsiniz. Beni yari yolda birakmadiniz, Allahta sizi yari yolda birakmasin.</p>
  </div>
</div>

<div class="review-card">
  <div class="review-avatar">EF</div>
  <div class="review-content">
    <div class="review-meta">
      <strong>enzo ferrari</strong>
      <span class="review-stars">★★★★★</span>
      <span class="review-date">bir yıl önce</span>
    </div>
    <p class="review-text">Allah razı olsun motor inecek denilen üç dört tane servise götürüp hala düzelmeyen arabamı bir saat icinde tamir edip canavar gibi yaptı usta. Gönül rahatlığıyla tavsiye ederim</p>
  </div>
</div>

<div class="review-card">
  <div class="review-avatar">OT</div>
  <div class="review-content">
    <div class="review-meta">
      <strong>ozan türköz</strong>
      <span class="review-stars">★★★★★</span>
      <span class="review-date">4 yıl önce</span>
    </div>
    <p class="review-text">Merhabalar, buradaki yorumları okuyarak buldum işletmeyi. Leon tsi aracımı hep yetkili servise götürmüştüm şimdiye kadar. İyi niyet garantisinin bitmesi ve yetkili servisin fiyatları iyice sisirmesi üzerine özel servis arayışına girdim. Alanında uzman görünüyorlar. Volkswagen grubunun her türlü tamirini yapıyorlar. Yani leb demeden derdinizi anlıyorlar ve çözüyorlar. Yetkili servise göre fiyatlar inanılmaz uygun. Yüzde 40 i desem yalan olmaz. Ha bir de kayıt sistemleri var. İs emri açıyorlar ve hangi bakımda ne değişti sisteme kayıt ediliyor. Bir problem yasamazsam sonradan burdan devam ederim diye düşünüyorum.</p>
  </div>
</div>

<div class="review-card">
  <div class="review-avatar">SG</div>
  <div class="review-content">
    <div class="review-meta">
      <strong>Serkan Gökçen</strong>
      <span class="review-stars">★★★★★</span>
      <span class="review-date">5 yıl önce</span>
    </div>
    <p class="review-text">2013 Scirocco'mun yıllık bakımlarını ve teknik sorunlarını burada yaptırıyorum. Ahmet ve Ömer kardeşler çok ilgililer araçtan da anlıyorlar. Orijinal yedek parça kullanıyorlar. Gönül rahatlığıyla gidebilirsiniz.</p>
  </div>
</div>

## Google Harita ve Yorumlar

<div class="map-embed">
  <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3010.3402005679377!2d28.631132100000002!3d41.0178125!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14cabb185ddf5cbb%3A0x6a61d71a45b9906a!2sY%C4%B1ld%C4%B1zlar%20Grup%20Volkswagen%2C%20Audi%2C%20Porsche%2C%20Bmw%20%C3%96zel%20Servis!5e0!3m2!1str!2str!4v1759931064197!5m2!1str!2str" width="100%" height="360" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
</div>
<p>Gerçek yorumlar: <a href="https://maps.app.goo.gl/gDpUepbcoo6AwpZN7" target="_blank">Google Haritalar</a></p>

## İletişim – Yıldızlar Grup Volkswagen Passat 120 Bin Bakım

<p><strong>Adres:</strong> Beykent Sanayi Sitesi Meriç Sk. No.: 179–180 Büyükçekmece / İstanbul</p>
<p><strong>Telefon:</strong> <a href="tel:05332623451">0533 262 34 51</a> – <strong>WhatsApp:</strong> <a href="https://api.whatsapp.com/send?phone=905332623451">Hemen Yaz</a></p>
<p><strong>Konum:</strong> <a href="https://maps.app.goo.gl/gDpUepbcoo6AwpZN7" target="_blank">Google Haritalar</a></p>
<p><strong>Çalışma Saatleri:</strong> Hafta içi 09:00–18:00, Cumartesi 09:00–14:00</p>

## Sık Sorulan Sorular

### Passat 120 bin bakımı ne kadar?

Passat 120 bin bakımı uygun maliyetler arasındadır. Bu fiyat 90.000 km bakımını da içerir. Timing zincir, DSG kavrama, su pompası, alternatör kontrolü araç başında yapılır. Değişim gerekiyorsa ek maliyet oluşur.

### Passat 120 bin bakımı ne içerir?

120 bin bakımı: 90.000 km bakımı + timing zincir kontrolü (araç başında), DSG kavrama kontrolü (araç başında), su pompası kontrolü (araç başında), alternatör kontrolü (araç başında), radyatör kontrolü (araç başında) içerir. Değişim gerekiyorsa ek maliyet oluşur.

### Passat timing zincir değişimi ne zaman yapılmalı?

Passat 2.0 TDI timing zincir kontrolü 100.000 km'de yapılmalı, değişim gerekiyorsa 100.000-180.000 km arasında yapılır. Özellikle 2010-2015 arası modellerde timing zincir gerginliği fabrikasyon kusurdur. Timing zincir değişimi uygun maliyetler; koparsa motor hasarı uygun maliyetler.

### Passat DSG kavrama değişimi ne zaman yapılmalı?

DQ250/DQ381 ıslak kavrama 120.000-180.000 km'de değiştirilmelidir. Şehir içi sık dur-kalkta kavrama aşınması %30 hızlanır. Erken değişim maliyeti uygun maliyetler; geç kalınırsa mekatronik hasarı eklenince uygun maliyetler.

### Passat su pompası değişimi ne zaman yapılmalı?

Passat su pompası kontrolü 120.000 km'de yapılmalı, değişim gerekiyorsa aynı anda yapılmalıdır. Su pompası arızası motor aşırı ısınmasına neden olur. Erken değişim maliyeti uygun maliyetler; motor hasarı durumunda uygun maliyetler.

### Passat alternatör değişimi ne zaman yapılmalı?

Passat alternatör kontrolü 120.000 km'de yapılmalı, değişim gerekiyorsa aynı anda yapılmalıdır. Alternatör arızası akü şarjını engeller. Erken değişim maliyeti uygun maliyetler; akü hasarı durumunda ek maliyet uygun maliyetler.

### Passat 120 bin bakım ne kadar sürer?

Passat 120 bin bakım süresi 2-3 gündür. Kontroller aynı gün yapılır, değişim gerekiyorsa 2-3 gün sürer.

### Passat 120 bin bakım garanti kapsamında mı?

Evet, Yıldızlar Grup'ta yapılan tüm 120 bin bakımlarda işçilik 12 ay, parçalarda 2 yıl garanti kapsamındadır.

### Passat 120 bin bakım için randevu nasıl alınır?

Passat 120 bin bakım için randevu almak için telefon (0533 262 34 51) veya WhatsApp üzerinden iletişime geçebilirsiniz. Ön kontrol araç başında yapılır.

### Passat 120 bin bakım fiyatı yetkili servisle karşılaştırıldığında ne kadar?

Yıldızlar Grup'ta Passat 120 bin bakım fiyatları yetkili servise göre %50-70 daha ekonomiktir. Aynı kalitede orijinal parça ve işçilikle hizmet sunuyoruz.

### Passat 120 bin bakımında hangi kontroller araç başında?

Passat 120 bin bakımında timing zincir kontrolü, DSG kavrama kontrolü, su pompası kontrolü, alternatör kontrolü ve radyatör kontrolü araç başında yapılır. Değişim gerekiyorsa ek maliyet oluşur.


